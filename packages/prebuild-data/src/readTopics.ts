import { tagSummaryLength } from "shared-lib";
import { BareTopic, getTagText } from "server-types";
...
function getSummary(paragraph: string[], isFirst: boolean): string | undefined {
  ...
  paragraph.forEach((x) => {
    const sentence = x + ".";
    if (sentence.length + rc.length + 1 > wanted) return;
    if (rc.length) {
      rc += " ";
    }
    rc += sentence;
...
export function readTopics(input: string[][]): BareTopic[] {
  ...
  const titles: string[] = input
    .map((paragraph) => paragraph[0].split(" ")[0])
    .concat(
      input.map((paragraph) => {
