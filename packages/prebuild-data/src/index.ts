...
import { BareTopic, TagId, getTagText } from "server-types";
...
const rootOutputDir = path.join(__dirname, "../../server-data");
const rootOutputTsDir = path.join(rootOutputDir, "src");
const rootOutputJsonDir = path.join(rootOutputDir, "json");
const rootPublicDir = path.join(__dirname, "../../ui-react/public");
...
function getOutputFile(fileName: string, ext = ".json"): string {
  ...
  const outputDir = ext === ".json" ? rootOutputJsonDir : rootOutputTsDir;
